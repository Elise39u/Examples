...
if (isset($_SESSION['Moved'])) {
    setcookie('Quest52', true, time() + 2147483647, '', '', '', true);
} else {
    $sql2 = sprintf("SELECT stat_id FROM npc_stats WHERE npc_id= 52");
    $query = mysqli_query($mysqli, $sql2);
    $row = $query->fetch_row();

    if (isset($_SESSION['Moved'])) {}
    elseif (in_array("13", $row)) {
        $update = "UPDATE npc_stats SET stat_id = 14 WHERE npc_id = 52";
        ...
        SELECT id FROM npc WHERE id = 52))",
            mysqli_real_escape_string($mysqli, $_SESSION['username']));
...
$smarty->display("tpl/NPC52.html.tpl");