...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (isset($row['item_id'])) {
        if($row['item_id'] == 108) {
            $_COOKIE['Quest19'] = true;
        }
    }
}

if (!isset($_COOKIE['Quest19'])) {
    setcookie('Quest19', false, time() + 2147483647, '', '', '', true);
}

if (isset($_COOKIE['Quest19'])) {
    if ($_COOKIE['Quest19'] == true) {
        if (isset($_SESSION['PageNpc19'])) {
            $_SESSION['PageNpc19']++;
        } else {
            $_SESSION['PageNpc19'] = 1;
        }
    }
}
...
$smarty->display("tpl/NPC19.html.tpl");