...
$sql2 = sprintf("SELECT stat_id FROM npc_stats WHERE npc_id= 56");
$query = mysqli_query($mysqli, $sql2);
$row = $query->fetch_row();

if (in_array("13", $row)) {
    $update = "UPDATE npc_stats SET stat_id = 14 WHERE npc_id = 56";
    ...
    $insert = sprintf("Insert Into party_members (party_id, npc_id) VALUES((SELECT id FROM party WHERE player_id=(
        ...
        SELECT id FROM npc WHERE id = 56))",
        mysqli_real_escape_string($mysqli, $_SESSION['username']));
...
$smarty->display("tpl/NPC56.html.tpl");