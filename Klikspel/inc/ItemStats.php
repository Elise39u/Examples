...
function getItem($itemID) {
    $conn = new mysqli('localhost', 'root', '', 'klikspel');;	$query = sprintf("SELECT name, type FROM items WHERE id = '%s'",
...
function createIfNotExistsItem($statName,$itemID) {
    $conn = new mysqli('localhost', 'root', '', 'klikspel');;
    $query = sprintf("SELECT count(content) FROM item_stats WHERE stat_id = (SELECT id FROM stats WHERE display_name = '%s' OR short_name = '%s') AND item_id = '%s'",
        mysqli_real_escape_string($conn, $statName),
        mysqli_real_escape_string($conn, $statName),
        mysqli_real_escape_string($conn, $itemID));
    $result = mysqli_query($conn, $query);
